...
import { Users, CheckCircle, XCircle, Clock, Mail, Phone, Award } from 'lucide-react'
...
export default function Applicants() {
  ...
  const recruiterJobs = mockJobOpenings.filter((job) => job.recruiterId === user?.id)
  ...
  const handleAction = (appId: string, newStatus: 'shortlisted' | 'rejected') => {
      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-5">
            <TabsTrigger value="all">All ({recruiterApplications.length})</TabsTrigger>
            {statuses.map((status) => {
              const count = recruiterApplications.filter((app) => app.status === status).length
              return (
          ...
          {statuses.map((status) => {
            const filteredApps = recruiterApplications.filter((app) => app.status === status)
            return (
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-foreground font-medium">{student.cgpa.toFixed(2)} CGPA</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
