...
import { mockJobOpenings, mockApplications, mockRecruiters } from '@/lib/mock-data'
...
export default function RecruiterDashboard() {
  ...
  const stats = {
    ...
    shortlisted: recruiterApplications.filter((a) => a.status === 'shortlisted').length,
    selected: recruiterApplications.filter((a) => a.status === 'selected').length,
  }

  // Chart data - applications by job
  const chartData = recruiterJobs.map((job) => ({
    name: job.role.substring(0, 10),
    applications: recruiterApplications.filter((app) => app.jobId === job.id).length,
    ctc: job.ctc,
  }))
        ...
        {chartData.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Number of applications received for each position</CardDescription>
            </CardHeader>
            <CardContent>
