...
import { mockApplications, mockJobOpenings } from '@/lib/mock-data'
      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-5">
            ...
            {statuses.map((status) => {
              const count = studentApplications.filter((app) => app.status === status).length
              return (
          ...
          {statuses.map((status) => {
            const filteredApps = applicationsWithDetails.filter((app) => app.status === status)
            return (
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-xl font-bold text-foreground">{app.job.role}</h3>
